...
#ifndef __FLOW_IPV6__
#define __FLOW_IPV6__
#endif
...
static int flow_scan(int fd, int op, struct flow_counters *cnt, FILE *out)
{
	...
	/* Reset counters before starting the iteration. */	//TODO why?
		...
		switch (op) {
			case OP_PURGE:
				...
				if( ( flow_to_remove(&key, &value) > 0 ) && 
					...
					cnt->active += 1 + bidirectional;
					cnt->tot += bidirectional;
					prev_key = key;
				}
