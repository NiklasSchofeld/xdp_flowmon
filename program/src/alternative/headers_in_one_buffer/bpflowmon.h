...
/* struct to hold parsed headers in scratch buffer */
// needed for alternative architecture (saving complete header in scratch buffer)
struct headers {
	__u64 ts;
	struct protocols prtotocols;
	union {
		struct iphdr ip4;
        struct ipv6hdr ip6;
	} layer3;
