...
struct {
    ...
    __type(key, int);
	__type(value, struct headers);
} flow_scratch_buffer SEC(".maps");
...
int bpflowmon(struct xdp_md *ctx)
{
    ...
    u64 ts;
    struct headers buffer = { 0 };

    ts = bpf_ktime_get_ns();
    buffer.ts = ts;
    err = bpf_map_update_elem(&flow_scratch_buffer, &key, &buffer, BPF_ANY);
    if (err)
        #ifdef __VERBOSE__
        bpf_printk("---BPF DEBUG--- ERROR: scratch_buffer update failed: %d\n", err);
        #endif
...
int parse_ip4(struct xdp_md *ctx)
{
    ...
    headers = bpf_map_lookup_elem(&flow_scratch_buffer, &key);
    if (!headers) {
        #ifdef __VERBOSE__
        bpf_printk("---BPF DEBUG--- ERROR: getting flow_scratch_buffer failed\n");
        #endif
        return DEFAULT_XDP_FAIL_ACTION;
    }
    ...
    #ifdef __VERBOSE__
    bpf_printk("---BPF DEBUG--- INFO: IP src addr = %d\n", iph->saddr);
    #endif
    #ifdef __VERBOSE__
    bpf_printk("---BPF DEBUG--- INFO: IP dst addr = %d\n", iph->daddr);
    #endif
    
    /**********HEADER PARSING**********/
    //TODO parse stats
    headers->prtotocols.l3 = IP;
    headers->layer3.ip4 = *iph;

    /*********************************/

    err = bpf_map_update_elem(&flow_scratch_buffer, &key, headers, BPF_ANY);    //TODO check error
    if (err)
        ;
...
int parse_ip6(struct xdp_md *ctx)
{
    ...
    /**********HEADER PARSING**********/
    //TODO parse stats
    headers->prtotocols.l3 = IPV6;
    headers->layer3.ip6 = *ip6h;
    /*********************************/

    bpf_map_update_elem(&flow_scratch_buffer, &key, headers, BPF_ANY);    //TODO check error
...
int parse_udp(struct xdp_md *ctx)
{
    ...
    /**********HEADER PARSING**********/
    //TODO parse stats
    headers->prtotocols.l4 = UDP;
    headers->layer4.udp = *udph;
    /*********************************/
    bpf_map_update_elem(&flow_scratch_buffer, &key, headers, BPF_ANY);
...
int parse_tcp(struct xdp_md *ctx)
{
        ...
        switch(options_len)   //my loop and memcpy tries caused too much verifier complains.
        {
            case 40:
                if ((void *)(options + 10) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[36] = *(options+9);
            case 36:
                if ((void *)(options + 9) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[32] = *(options+8);
            case 32:
                if ((void *)(options + 8) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[28] = *(options+7);
            case 28:
                if ((void *)(options + 7) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[24] = *(options+6);
            case 24:
                if ((void *)(options + 6) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[20] = *(options+5);
            case 20:
                if ((void *)(options + 5) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[16] = *(options+4);
            case 16:
                if ((void *)(options + 4) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[12] = *(options+3);
            case 12:
                if ((void *)(options + 3) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[8] = *(options+2);
            case 8:
                if ((void *)(options + 2) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[4] = *(options+1);
            case 4:
                if ((void *)(options + 1) > data_end)
                    return DEFAULT_XDP_FAIL_ACTION;
                headers->tcp_opts.option_words[0] = *(options);
                break;
            default:
                break;
        }
    ...
    /**********HEADER PARSING**********/
    //TODO parse stats
    headers->prtotocols.l4 = TCP;
    headers->layer4.tcp = *tcph;
    /*********************************/
    bpf_map_update_elem(&flow_scratch_buffer, &key, headers, BPF_ANY);
...
int parse_icmp(struct xdp_md *ctx)
{
    ...
    /**********HEADER PARSING**********/
    //TODO parse stats
    headers->prtotocols.l4 = ICMP;
    headers->layer4.icmp4 = *icmph;
    /*********************************/
    bpf_map_update_elem(&flow_scratch_buffer, &key, headers, BPF_ANY);
...
int parse_icmpv6(struct xdp_md *ctx)
{
    ...
    /**********HEADER PARSING**********/
    //TODO parse stats
    headers->prtotocols.l4 = ICMPV6;
    headers->layer4.icmp6 = *icmp6h;
    /*********************************/
    bpf_map_update_elem(&flow_scratch_buffer, &key, headers, BPF_ANY);
...
int exit_program(struct xdp_md *ctx)
{
    ...
    /* construct flow_id */ 
        /* saddr, daddr, proto */
    switch (headers->prtotocols.l3)
    {
        case IP:
            flow_id.saddr.v4 = headers->layer3.ip4.saddr;
            flow_id.daddr.v4 = headers->layer3.ip4.daddr;
            flow_id.proto    = headers->layer3.ip4.protocol;
            ip_version = 4;
            break;
        case IPV6:
            memcpy(flow_id.saddr.v6, headers->layer3.ip6.saddr.in6_u.u6_addr8, 16);
            memcpy(flow_id.daddr.v6, headers->layer3.ip6.daddr.in6_u.u6_addr8, 16);
            flow_id.proto = headers->layer3.ip6.nexthdr;
            ip_version = 6;
            break;
        default:
            fail = true;
        ...
        /* sport, dport */
    switch (headers->prtotocols.l4)
    {
        case UDP:
            flow_id.sport = headers->layer4.udp.source;
            flow_id.dport = headers->layer4.udp.dest;
            break;
        case TCP:
            flow_id.sport = headers->layer4.tcp.source;
            flow_id.dport = headers->layer4.tcp.dest;
            break;
        case ICMP:
            //TODO maybe not suitable for flow monitoring
            icmph = &headers->layer4.icmp4;
    ...
    if (!flow_info) {
        ...
        if (ip_version == 4)
            info.version       = headers->layer3.ip4.version;
        else if (ip_version == 6)
            info.version       = headers->layer3.ip6.version;
    ...
    update_frame_stats(flow_info, headers->ts);
    // bpf_map_update_elem(&flow_stats, &flow_id, flow_info, BPF_ANY);   //when updating the map also here, 40 loops for options parsing are possible
    
    /* write stats to flow_info */
    switch (headers->prtotocols.l3)
    {
        case IP:
            ip4_update_stats(&headers->layer3.ip4, flow_info);
            set_ethaddr(ctx, 4, &headers->layer3.ip4.daddr, &action);
            break;
        case IPV6:
            ip6_update_stats(&headers->layer3.ip6, flow_info);
            set_ethaddr(ctx, 6, &headers->layer3.ip6.daddr, &action);
            break;
        case ARP:
            action = XDP_PASS;
            break;
    ...
    switch (headers->prtotocols.l4)
    {
        case UDP:
            break;
        case TCP:
            tcp_update_stats(&headers->layer4.tcp, flow_info, headers->tcp_opts.option_words);
            break;
        case ICMP:
            break;
        case ICMPV6:
