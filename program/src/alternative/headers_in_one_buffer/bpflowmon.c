...
int init_jmp_table(struct bpf_object *obj, struct params *cfg, int *main_prog_fd)
{
	...
	cfg->protocols[EXIT_PROGRAM] = true;		//must always be used in jump table
...
char *add_protocol(char *protos, struct params *cfg)
{
	...
	if(!cfg->restricted_protos) {
		for(int i=0; i<MAX_PROGS; i++) {
			if (i != EXIT_PROGRAM && i != FALL_THROUGH)
...
int main(int argc, char **argv)
{
	...
	{	
		...
		if(cfg.map_reuse && strcmp(bpf_map__name(map), "flow_stats") == 0)	
		;	//skip flow_stats map if it is reused
		else 
		{	/* unpin existing map */
