...
struct {
    ...
    __type(key, int);
	__type(value, __u64);
} timestamp_scratch_buffer SEC(".maps");
...
struct {
    ...
    __type(key, int);
	__type(value, int);
} protocol_l3_scratch_buffer SEC(".maps");
...
struct {
    ...
    __type(key, int);
	__type(value, int);
} protocol_l4_scratch_buffer SEC(".maps");

struct {
    __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
    __uint(max_entries, 1);
    __type(key, int);
	__type(value, struct iphdr);
} ip_scratch_buffer SEC(".maps");
...
} ip6_scratch_buffer SEC(".maps");

struct {
    __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
    __uint(max_entries, 1);
    __type(key, int);
	__type(value, struct tcph);
} tcp_scratch_buffer SEC(".maps");
#ifdef __FLOW_TCP_OPTS__
struct {
    __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
    __uint(max_entries, 1);
    __type(key, int);
	__type(value, struct tcp_options_words);
} tcp_opts_scratch_buffer SEC(".maps");
#endif
...
struct {
    ...
    __uint(max_entries, 1);
    __type(key, int);
	__type(value, struct udphdr);
} udp_scratch_buffer SEC(".maps");
...
} icmp_scratch_buffer SEC(".maps");

struct {
    __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
    __uint(max_entries, 1);
    __type(key, int);
	__type(value, struct icmp6hdr);
} icmp6_scratch_buffer SEC(".maps");
...
static __always_inline void parse_l3_protocol(struct xdp_md *ctx, __u16 proto)
{
    ...
    /* fall through */
    bpf_tail_call(ctx, &jmp_table, EXIT);   //if a protocol is not set from user space, the EXIT program is called to avoid adding data to the flow_stats map for this packet
...
int bpflowmon(struct xdp_md *ctx)
{
    ...
    ts = bpf_ktime_get_ns();
    err = bpf_map_update_elem(&timestamp_scratch_buffer, &key, &ts, BPF_ANY);
    if (err) {
        #ifdef __DEBUG__
    ...
    err = bpf_map_update_elem(&protocol_l3_scratch_buffer, &key, &value, BPF_ANY);      //init with 0 to avoid conflicts with assignments of previous program runs
    err = bpf_map_update_elem(&protocol_l4_scratch_buffer, &key, &value, BPF_ANY);      //init with 0 to avoid conflicts with assignments of previous program runs
    if (err) {
        #ifdef __DEBUG__
        bpf_printk("---BPF DEBUG--- ERROR: scratch_buffer update failed: %d\n", err);
        #endif
...
int parse_ip4(struct xdp_md *ctx)
{
    ...
    /**********HEADER PARSING**********/
    err = bpf_map_update_elem(&protocol_l3_scratch_buffer, &key, &protocol, BPF_ANY);    //TODO check error
    if (err)
        ;
    err = bpf_map_update_elem(&ip_scratch_buffer, &key, iph, BPF_ANY);    //TODO check error
    if (err)
        ;
    /*********************************/
...
int parse_ip6(struct xdp_md *ctx)
{
    ...
    if (err)
        ;
    err = bpf_map_update_elem(&ip6_scratch_buffer, &key, ip6h, BPF_ANY);    //TODO check error
    if (err)
        ;
    /*********************************/
...
int parse_udp(struct xdp_md *ctx)
{
    ...
    /**********HEADER PARSING**********/
    err = bpf_map_update_elem(&protocol_l4_scratch_buffer, &key, &protocol, BPF_ANY);    //TODO check error
    if (err)
        ;
    err = bpf_map_update_elem(&udp_scratch_buffer, &key, udph, BPF_ANY);    //TODO check error
    if (err)
        ;
    /*********************************/
...
int parse_tcp(struct xdp_md *ctx)
{
    ...
    if (err)
        ;
    err = bpf_map_update_elem(&tcp_scratch_buffer, &key, tcph, BPF_ANY);    //TODO check error
    if (err)
        ;
    #ifdef __FLOW_TCP_OPTS__
...
int parse_icmp(struct xdp_md *ctx)
{
    ...
    if (err)
        ;
    err = bpf_map_update_elem(&icmp_scratch_buffer, &key, icmph, BPF_ANY);    //TODO check error
    if (err)
        ;
    /*********************************/
...
int parse_icmpv6(struct xdp_md *ctx)
{
    ...
    if (err)
        ;
    err = bpf_map_update_elem(&icmp6_scratch_buffer, &key, icmp6h, BPF_ANY);    //TODO check error
    if (err)
        ;
    /*********************************/
...
int exit_program(struct xdp_md *ctx)
{
    ...
    proto_l3 = bpf_map_lookup_elem(&protocol_l3_scratch_buffer, &key);
    if (!proto_l3) {
        #ifdef __DEBUG__
    ...
    proto_l4 = bpf_map_lookup_elem(&protocol_l4_scratch_buffer, &key);
    if (!proto_l4) {
        #ifdef __DEBUG__
        bpf_printk("---BPF DEBUG--- ERROR: no value in scratch buffer\n");
        #endif
    ...
    {
        case IP:
            iph = bpf_map_lookup_elem(&ip_scratch_buffer, &key);
            if (!iph) {
                #ifdef __DEBUG__
                bpf_printk("---BPF DEBUG--- ERROR: no value in scratch buffer\n");
                #endif
            ...
            break;
        case IPV6:
            ip6h = bpf_map_lookup_elem(&ip6_scratch_buffer, &key);
            if (!ip6h) {
                #ifdef __DEBUG__
                bpf_printk("---BPF DEBUG--- ERROR: no value in scratch buffer\n");
                #endif
    ...
    {
        case UDP:
            udph = bpf_map_lookup_elem(&udp_scratch_buffer, &key);
            if (!udph) {
                #ifdef __DEBUG__
                bpf_printk("---BPF DEBUG--- ERROR: no value in scratch buffer\n");
                #endif
            ...
            break;
        case TCP:
            tcph = bpf_map_lookup_elem(&tcp_scratch_buffer, &key);
            if (!tcph) {
                #ifdef __DEBUG__
                bpf_printk("---BPF DEBUG--- ERROR: no value in scratch buffer\n");
                #endif
        ...
        case ICMP:
            ...
            icmph = bpf_map_lookup_elem(&icmp_scratch_buffer, &key);
            if (!icmph) {
                #ifdef __DEBUG__
                bpf_printk("---BPF DEBUG--- ERROR: no value in scratch buffer\n");
                #endif
        ...
        case ICMPV6:
            //TODO maybe not suitable for flow monitoring 
            icmp6h = bpf_map_lookup_elem(&icmp6_scratch_buffer, &key);
            if (!icmp6h) {
                #ifdef __DEBUG__
                bpf_printk("---BPF DEBUG--- ERROR: no value in scratch buffer\n");
                #endif
    ...
    ts = bpf_map_lookup_elem(&timestamp_scratch_buffer, &key);
    if (!ts) {
        #ifdef __DEBUG__
        bpf_printk("---BPF DEBUG--- ERROR: no value in scratch buffer\n");
        #endif
