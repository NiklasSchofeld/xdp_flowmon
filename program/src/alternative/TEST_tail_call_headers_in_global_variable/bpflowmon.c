...
char *add_protocol(char *protos, struct params *cfg)
{
	...
	if(!cfg->restricted_protos) {
		for(int i=0; i<MAX_PROGS; i++) {
			if (i != FLOW_ID_FINISH && i != FALL_THROUGH)
		...
		switch(opt)
		{
			...
			case 'm':	//xdp flags
				...
				while (optarg != NULL)
				{
					...
					else if (strcmp(optarg, "hardware") == 0 || strcmp(optarg, "offload") == 0)
...
int main(int argc, char **argv)
{
	...
	/* run test */
	// err = bpf_prog_test_run(main_prog_fd, 1, &pkt_v4, sizeof(pkt_v4), &data_out, &size, &retval, &duration);
	err = bpf_prog_test_run(main_prog_fd, 100000, &data_in, sizeof(pkt_v4), &data_out, &size, &retval, &duration);
	// err = bpf_prog_test_run_xattr(&test_attr);
	// err = bpf_prog_test_run_opts(main_prog_fd, &opts);
	// err = bpf_prog_test_run_xdp(&test_attr);
	if (err) {
	...
	__u32 duration2; 
	__u32 avg_duration = 0;
	for(int i=0; i<10000000; i++)
	{
		err = bpf_prog_test_run(main_prog_fd, 1, &data_in, sizeof(pkt_v4), &data_out, &size, &retval, &duration2);
		avg_duration += duration2;
	}

	avg_duration = avg_duration / 10000000;
	...
	printf("Successfully ran 1 test run.\n");
	printf("average duration: %d ns\n", duration);
	printf("return value: %d\n", retval);

	printf("Successfully ran 2 test run.\n");
	printf("average duration: %d ns\n", avg_duration);
	printf("return value: %d\n", retval);
	exit(0);
