...
{
    #ifdef __VERBOSE__
    bpf_printk("---BPF DEBUG--- INFO: ip4_update_stats\n");
    #endif
    int idx, len;
    __u8 ttl;
...
static __always_inline void parse_l4_protocol_info(struct xdp_md *ctx)
{
    switch (proto_l4) {
        case IPPROTO_ICMP:
            bpf_tail_call(ctx, &jmp_table, ICMP_INFO);
            break;
        case IPPROTO_ICMPV6:
            bpf_tail_call(ctx, &jmp_table, ICMPV6_INFO);
            break;
        case IPPROTO_TCP:
            bpf_tail_call(ctx, &jmp_table, TCP_INFO);
            break;
        case IPPROTO_UDP:
            bpf_tail_call(ctx, &jmp_table, UDP_INFO);
            break;
        default:
            break;
    }
...
int flow_info_finish(struct xdp_md *ctx)
{
    ...
    bpf_map_update_elem(&flow_stats, &flow_id, &flow_info, BPF_ANY);
...
int flow_id_finish(struct xdp_md *ctx)
{
    ...
    bool fail = false, valid_flow_id = true;
    ...
    #ifdef __VERBOSE__
    bpf_printk("---BPF DEBUG--- INFO: saddr = %d\n",flow_id.saddr.v4);
    bpf_printk("---BPF DEBUG--- INFO: daddr = %d\n",flow_id.daddr.v4);
    bpf_printk("---BPF DEBUG--- INFO: sport = %d\n",flow_id.sport);
    bpf_printk("---BPF DEBUG--- INFO: dport = %d\n",flow_id.dport);
    bpf_printk("---BPF DEBUG--- INFO: proto = %d\n",flow_id.proto);
    #endif
    ...
    if (!flow_info_p) { //verifier still needs the check to be sure that flow_info is not null.
        #ifdef __DEBUG__
