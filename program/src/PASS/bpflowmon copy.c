...
__u32 pkts=0;
	if (cfg.test) {
		...
		/* count pkts */
		// pkts = (int) count_pkts(map_fd) + skel->bss->pkts;
		pkts = skel->bss->pkts;
		printf("pkts: %ud\n", skel->bss->pkts);
		printf("pkts: %d\n", skel->bss->pkts);
		printf("pkts: %u\n", skel->bss->pkts);
		printf("pkts: %d\n", (int) skel->bss->pkts);
		bpf_map_lookup_elem(bpf_map__fd(skel->maps.pkts_map), &key, &all_pkts);
		printf("pkts: %d\n", all_pkts);
		printf("pkts: %u\n", all_pkts);
		printf("pkts: %d\n", (int) all_pkts);
	...
	/* run test */
	err = bpf_prog_test_run(main_prog_fd, 2, &data_in, sizeof(pkt_v4), &data_out, &size, &retval, &duration);
	if (err) {
	...
	// 	// avg_duration = avg_duration / i+1;
	// }

	// avg_duration = avg_duration / 10000;
